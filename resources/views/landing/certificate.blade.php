            ...
            <div class="grid lg:grid-cols-[1fr_320px] items-stretch divide-y lg:divide-y-0 lg:divide-x divide-white/40 dark:divide-white/10">
                ...
                <div class="p-6 sm:p-8 lg:p-10 min-w-0 flex flex-col justify-between">
                    <div class="space-y-8">
                        ...
                        <div class="bg-white/50 dark:bg-white/5 backdrop-blur-md p-4 sm:p-6 rounded-2xl border border-white/50 dark:border-white/10">
                            <img src="{{ img_url($certificate->gambar) }}" alt="{{ $certificate->nama }}"
                                 class="w-full h-auto object-contain rounded-xl bg-white/80 dark:bg-[#0a0a0f]" loading="lazy">
                        </div>
                        ...
                        @if ($certificate->desk)
                            <div class="pt-6 border-t border-white/40 dark:border-white/10">
                                ...
                                <p class="text-sm sm:text-[0.95rem] text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line" x-text='L(@json($certificate->desk), @json($certificate->desk_idn))'>
                                    {{ $certificate->desk }}
                                </p>
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            @if ($certificate->link)
                                <a href="{{ $certificate->link }}" target="_blank" rel="noopener noreferrer"
                                   class="w-full mt-6 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-accent/90 backdrop-blur-md border border-white/20 text-white text-sm font-semibold transition-all duration-300 ease-out hover:bg-blue-600 hover:-translate-y-1">
