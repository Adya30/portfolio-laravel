@php
    ...
    $footerLinkClasses = 'text-sm text-slate-600 dark:text-slate-400 hover:text-accent dark:hover:text-accent-light transition-colors duration-200';
    $footerHeadingClasses = 'font-poppins font-bold text-sm uppercase tracking-wider mb-6 text-slate-900 dark:text-white';
@endphp

<footer class="relative z-10 mt-8 border-t border-line bg-white pt-16 pb-12 px-6 dark:border-white/10 dark:bg-[#0c0c14] md:pt-20 md:pb-16">
    <div class="max-w-6xl mx-auto">
        <div class="grid md:grid-cols-3 gap-12 md:gap-16 lg:gap-24">

            <div class="space-y-6">
                <span class="font-poppins font-bold text-lg text-slate-900 dark:text-white">{{ $footerName }}</span>
                <p class="text-sm leading-relaxed max-w-xs text-slate-600 dark:text-slate-400"
            ...
            <div>
                <h3 class="{{ $footerHeadingClasses }}" x-text="t('quickLinks')">Quick Links</h3>
                <ul class="space-y-3.5">
                    @foreach ($quickLinks as $link)
                        <li>
                            @if (! empty($link['external']))
                                <a href="{{ $link['href'] }}" class="{{ $footerLinkClasses }}">
            ...
            <div>
                <h3 class="{{ $footerHeadingClasses }}" x-text="t('connect')">Connect</h3>
                <div class="space-y-4">
                    @foreach ($socials as $social)
                        <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer"
                           class="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400 hover:text-accent dark:hover:text-accent-light transition-colors duration-200">
        ...
        <div class="mt-20 pt-10 border-t border-line dark:border-white/10">
